...
CREATE TABLE `task` (
  ...
  `project_code_snapshot` varchar(100) DEFAULT NULL COMMENT '项目编号快照，不作为关系键',
...
CREATE TABLE `work_hour_record` (
  ...
  `employee_no_snapshot` varchar(100) DEFAULT NULL COMMENT '报工时外包工号快照',
  ...
  `vendor_company_code_snapshot` varchar(100) DEFAULT NULL COMMENT '报工时外包公司编码快照',
  `vendor_company_name_snapshot` varchar(200) DEFAULT NULL COMMENT '报工时外包公司名称快照',
  ...
  `task_id` varchar(32) DEFAULT NULL COMMENT '关联开发任务ID；无点亮任务异常报工可空',
  `req_id` varchar(32) DEFAULT NULL COMMENT '关联需求ID，可空',
  `project_id_snapshot` varchar(32) DEFAULT NULL COMMENT '报工时关联项目ID快照，可空',
  `project_code_snapshot` varchar(100) DEFAULT NULL COMMENT '报工时项目编号快照，可空',
  `system_id` varchar(32) DEFAULT NULL COMMENT '研发系统快照（关联 admin_sm_logic_sys.SYS_ID）',
  `module_id` varchar(32) DEFAULT NULL COMMENT '系统模块快照（关联 dev_system_module.id）',
  `source_type` varchar(20) DEFAULT NULL COMMENT '来源（自动生成/手动填报/知微迁移/知微灰度）',
  `source_system` varchar(50) DEFAULT NULL COMMENT '明细实际来源系统',
  `source_record_id` varchar(100) DEFAULT NULL COMMENT '来源系统记录ID，迁移/双系统接入时使用',
  `source_batch_id` varchar(32) DEFAULT NULL COMMENT '来源同步/迁移/灰度批次ID（关联 sync_batch_record.id，可空）',
  `source_rule_version` varchar(100) DEFAULT NULL COMMENT '生成、导入或去重时采用的业务规则版本',
  `gray_route_decision_id` varchar(32) DEFAULT NULL COMMENT '灰度来源裁决ID（关联 gray_route_decision.id，可空）',
  `generation_batch_no` varchar(50) DEFAULT NULL COMMENT '默认工时生成批次号',
  `idempotency_key` varchar(200) DEFAULT NULL COMMENT '幂等键；自动生成为租户+用户+日期+任务占位值',
  `exception_flag` tinyint DEFAULT 0 COMMENT '是否异常报工（0否/1是）',
  `exception_type` varchar(30) DEFAULT NULL COMMENT '无点亮任务/考勤缺失/映射失败/其他',
  `workload_confirm_status` tinyint DEFAULT 0 COMMENT '知脉工作量确认/封板快照（0未确认/1已确认）',
  `lock_status` tinyint DEFAULT 0 COMMENT '是否锁定（0否/1是）',
  `lock_time` datetime DEFAULT NULL COMMENT '锁定时间',
  `settle_status` tinyint DEFAULT NULL COMMENT '结算状态',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `data_tenant_id` varchar(32) NOT NULL DEFAULT '1' COMMENT '数据所属租户id',
  `created_by` varchar(32) DEFAULT NULL COMMENT '创建人ID',
  ...
  `delete_flag` tinyint NOT NULL DEFAULT 0 COMMENT '逻辑删除标识（0正常/1删除）',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uk_tenant_idempotency_key` (`data_tenant_id`, `idempotency_key`),
  UNIQUE KEY `uk_tenant_src_rec` (`data_tenant_id`, `source_system`, `source_record_id`),
  KEY `idx_user_date` (`user_id`, `work_date`),
  KEY `idx_task_date` (`task_id`, `work_date`),
  KEY `idx_settle_lock` (`settle_status`, `lock_status`),
  KEY `idx_gray_decision` (`gray_route_decision_id`),
...
CREATE TABLE `attendance_sync_record` (
  `id` varchar(32) NOT NULL COMMENT '主键',
  `source_system` varchar(50) DEFAULT NULL COMMENT '考勤来源系统',
  `source_record_id` varchar(100) DEFAULT NULL COMMENT '来源记录ID',
  `source_user_code` varchar(100) DEFAULT NULL COMMENT '来源工号；映射失败时仍必须落库',
  `user_id` varchar(32) DEFAULT NULL COMMENT '映射后的用户ID，可空；关联 admin_sm_user.USER_ID',
  `work_date` date DEFAULT NULL COMMENT '考勤日期',
  `attendance_minutes` int DEFAULT NULL COMMENT '考勤时长（分钟）',
  `record_version` varchar(100) DEFAULT NULL COMMENT '来源记录版本；无版本时使用来源更新时间+载荷摘要',
  `supersedes_record_id` varchar(32) DEFAULT NULL COMMENT '被本记录替代的上一版本ID，可空',
  `source_update_time` datetime DEFAULT NULL COMMENT '来源最后更新时间，用于覆盖判断',
  `mapping_status` varchar(20) DEFAULT NULL COMMENT '已映射/待映射/映射冲突',
  `sync_status` varchar(20) DEFAULT NULL COMMENT '成功/失败/待处理',
  `error_code` varchar(50) DEFAULT NULL COMMENT '错误码',
  `error_message` varchar(1000) DEFAULT NULL COMMENT '错误原因',
  `raw_payload` json DEFAULT NULL COMMENT '来源数据快照',
  `sync_time` datetime DEFAULT NULL COMMENT '同步时间',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `data_tenant_id` varchar(32) NOT NULL DEFAULT '1' COMMENT '数据所属租户id',
  `created_by` varchar(32) DEFAULT NULL COMMENT '创建人ID',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uk_tenant_src_rec_ver` (`data_tenant_id`, `source_system`, `source_record_id`, `record_version`),
  KEY `idx_user_date_update` (`user_id`, `work_date`, `source_update_time`),
  KEY `idx_map_sync_sts` (`mapping_status`, `sync_status`)
...
CREATE TABLE `work_hour_config` (
  ...
  `delete_flag` tinyint NOT NULL DEFAULT 0 COMMENT '逻辑删除标识（0正常/1删除）',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uk_tenant_cfg_key_eff` (`data_tenant_id`, `config_key`, `effective_start_time`)
