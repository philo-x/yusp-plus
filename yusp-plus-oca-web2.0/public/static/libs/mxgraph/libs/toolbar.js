...
Toolbar.prototype.init = function () {
  ...
  // var elts = this.addItems(['-', 'saveData', 'relayout', '-']);
  // elts[1].setAttribute('title', mxResources.get('save'));
  // elts[2].setAttribute('title', '重新布局');

  // 增加连线、空白节点、模板节点
  // var insertMenu = this.addMenu('', mxResources.get('insert') + ' (' + mxResources.get('doubleClickTooltip') + ')', true, 'insert', null, true);
  // this.addDropDownArrow(insertMenu, 'geSprite-plus', 38, 48, -4, -3, 36, -8);
};
Toolbar.prototype.addCustomItems = function (items) {
  var sidebar = new Sidebar(this.editorUi, this.container);
  sidebar.thumbWidth = 22;
  sidebar.thumbHeight = 22;
  sidebar.showTooltips = false;
  var tmp = [];
  var _this = this;
  items.forEach(function (item) {
    if (item === '-') {
      tmp.length > 0 && sidebar.addItems(tmp);
      tmp = [];
      _this.addItems(['-']);
    } else {
      tmp.push(item);
